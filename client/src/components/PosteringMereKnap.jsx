        ...
        <div ref={menuRef} className={`${Styles.posteringMereMenu} ${closing ? Styles.closing : Styles.opening}`}>
          {!undermenuValg ? (
            <>
              ...
              {(user?.isAdmin || (user?.id === postering?.brugerID && !postering.låst)) && <button onClick={() => handleValg('rediger')}><span><SquarePen className={Styles.posteringMereInnerKnapIcon}/> Ret</span><ChevronRight className={Styles.posteringMereInnerKnapIcon} style={{marginRight: 0}}/></button>}
              {(user?.isAdmin || user?.id === postering?.brugerID) && <button onClick={() => handleValg('satser')}><span><Sigma className={Styles.posteringMereInnerKnapIcon}/> Beregning</span></button>}
              {(user?.isAdmin || user?.id === postering?.brugerID) && <button onClick={() => handleValg('vend')}><span><FlipHorizontal className={Styles.posteringMereInnerKnapIcon}/> Vend</span></button>}
              ...
              {posteringBetalt === 0 && (user?.isAdmin || (user?.id === postering?.brugerID && !postering?.låst)) && <button onClick={() => handleValg('slet')}><span><Trash2 className={Styles.posteringMereInnerKnapIcon}/> Slet</span></button>}
            </>
          ) : (
